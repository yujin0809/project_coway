...
if (pathname.includes('lifeelc')) objectData = lifeEleData;
if (pathname.includes('waterelc')) objectData = waterEleData;
if (pathname.includes('berexelc')) objectData = berexEleData;
if (pathname.includes('refurb')) objectData = refurbData;
...
const lifeElc = () => {
    ...
    const showItem = (data) => {
        ...
        data.forEach((data, idx) => {
            ...
            // 상세페이지 url 설정
            a.setAttribute('href', `/page/products/lifeelc/lifeelcDetail.html`);
        ...
        compareCompleBtn.addEventListener('click', (e) => {
            // showPopup('현재 가입하신 SNS 및 휴대전화 번호로 비교된 결과가 전달됩니다.');
            showPopup2('현재 가입하신 SNS 및 휴대전화 번호로 비교된 결과가 전달됩니다.', '뭐가제목?');
            ...
            modal2.innerHTML = '';
            // 주방 / 생활가전
            if (location.pathname.includes('lifeelc.html')) {
                let divArray = [];
...
// 매장소개
const introduce = () => {
    // box2  scroll  add(active)
    const $scrollLi = get('.introduce .inner .sub-text-image li:nth-child(1)');
    const $scrollLi2 = get('.introduce .inner .sub-text-image li:nth-child(2)');
    const $scrollLi3 = get('.introduce .inner .sub-text-image li:nth-child(3)');

    // $scrollLi2.classList.add("active");
    window.addEventListener('scroll', (e) => {
        let st = window.scrollY || document.documentElement.scrollTop;
        if (st >= 600) {
            $scrollLi.classList.add('active');
        } else {
            $scrollLi.classList.remove('active');
        }
        if (st >= 1100) {
            $scrollLi2.classList.add('active');
        } else {
            $scrollLi2.classList.remove('active');
        }
        if (st >= 1800) {
            $scrollLi3.classList.add('active');
        } else {
            $scrollLi3.classList.remove('active');
        }
    });

    const $storeUl = get('.introduce .store-store .store-slide-box .to-store');
    const imgMake = (src, name, address) => {
        ...
        $storeUl.append(li);
        li.append(img2, span, p1, p2);
        p1.classList.add('to-store-name');
        p2.classList.add('to-store-address');
        img2.setAttribute('src', src);
        span.textContent = '코웨이 공식 직영 매장';
        p1.textContent = name;
        p2.textContent = address;
        const $storSlideLi = getAll('.introduce .store-store .store-slide-box .to-store li');
    };
    stn.forEach((item) => {
        imgMake(item.img, item.name, item.address);
    });
    const $storSlideLi = getAll('.introduce .store-store .store-slide-box .to-store li');
    const $storSlideBtn = getAll('.introduce .store-store .store-slide-box .to-store button');
    const $storeImg = getAll('.introduce .store-store .store-slide-box .to-store li img');
    const $storeName = getAll('.introduce .store-store .store-slide-box .to-store li .to-store-name');
    const $storeAddress = getAll('.introduce .store-store .store-slide-box .to-store li .to-store-address');
    const $introPrev = get('.introduce .store-store .store-slide-box .to-store-prev');
    const $introNext = get('.introduce .store-store .store-slide-box .to-store-next');
    $storeImg.forEach((img, i) => {
        img.setAttribute('src', stn[i].img);
    });
    $storeName.forEach((title, i) => {
        title.textContent = stn[i].name;
    });
    $storeAddress.forEach((address, idx) => {
        address.textContent = stn[idx].address;
    });
    let wid = 1;
    $introNext.addEventListener('click', (e) => {
        wid++;
        $storSlideLi.forEach((li, i) => {
            li.style.transform = `translateX(-${wid * 820 - 490}px)`;
            let tran = li.style.transform;
            if (tran >= `translateX(-6890px)`) {
                $introNext.style.display = 'none';
            } else if (tran <= `translateX(-6070px)`) {
                $introPrev.style.display = 'block';
            }
        });
    });
    // 넥스트가 마지막일때 프리브를 눌러야지 넥스트가 블럭
    $introPrev.addEventListener('click', (e) => {
        ...
        if (wid > 0) {
            $storSlideLi.forEach((li, i) => {
                li.style.transform = `translateX(-${wid * 820 - 490}px)`;
                // console.log(li.style.transform);
                // 프리브가 없을때 넥스트를 눌러야지 프리브가 블럭
                $introNext.style.display = 'block';
            });
        } else {
            $storSlideLi.forEach((li, i) => {
                li.style.transform = `translateX(495px)`;
                $introPrev.style.display = 'none';
            });
        }
    });
};
...
const subInit = () => {
    ...
    // 주방/생활가전 - 상세페이지
    if (location.pathname.split('/').pop() === 'lifeelcDetail.html') {
        lifeElcDetail();
    }
    // 룰루비데/연수기
